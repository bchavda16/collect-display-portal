...
export default function AdminOrdersPage() {
  ...
  const readMutation = useMutation({
    mutationFn: async ({ orderIds, isRead }: any) => {
      const r = await fetch("/api/orders/read", { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ orderIds, isRead }) })
      return r.json()
    },
    onSuccess: () => { qc.invalidateQueries({queryKey:["admin-orders"]}); qc.invalidateQueries({queryKey:["admin-badges"]}); setSelected(new Set()) },
  })

  const editItemMutation = useMutation({
    mutationFn: async ({ orderId, itemId, unitCostPence, quantity }: any) => {
      const r = await fetch("/api/orders/"+orderId+"/items", { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ itemId, unitCostPence, quantity }) })
      return r.json()
    },
  ...
  const deleteItemMutation = useMutation({
    mutationFn: async ({ orderId, itemId }: any) => {
      const r = await fetch("/api/orders/"+orderId+"/items", { method:"DELETE", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ itemId }) })
      return r.json()
    },
    onSuccess: (d) => {
  ...
  const handleExpand = async (orderId: string, isRead: boolean) => {
    if (expanded === orderId) { setExpanded(null); return }
    setExpanded(orderId)
    if (!isRead) {
      await fetch("/api/orders/read", { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ orderIds: [orderId], isRead: true }) })
  ...
  const S: any = {
    ...
    th: {background:"#F4F5F7",fontSize:10,fontWeight:600,textTransform:"uppercase" as const,letterSpacing:".06em",color:"#8888AA",padding:"10px 14px",textAlign:"left" as const,borderBottom:"1px solid rgba(0,0,0,.08)"},
    td: {padding:"12px 14px",fontSize:13,borderBottom:"1px solid rgba(0,0,0,.06)",verticalAlign:"middle" as const},
    ...
    btnPink: {display:"inline-flex",alignItems:"center",justifyContent:"center",padding:"9px 18px",background:"#88dde1",color:"#0a1420",border:"none",borderRadius:8,fontSize:13,fontWeight:700,cursor:"pointer"},
  ...
  const chipStyle = (active: boolean) => ({
    ...
    background:active?"#88dde1":"#F4F5F7",color:active?"#0a1420":"#4A4A6A",
  ...
  return (
    <div style={S.page}>
      ...
      {/* Bulk action bar */}
      {selected.size > 0 && (
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#f0fafb",border:"1px solid #88dde1",borderRadius:10,marginBottom:14}}>
          <span style={{fontSize:13,fontWeight:600,color:"#1a9da3"}}>{selected.size} selected</span>
          <button style={{...S.btnSm,background:"#EAFAF3",color:"#0EA572",border:"1px solid rgba(14,165,114,.2)"}}
            onClick={()=>readMutation.mutate({orderIds:Array.from(selected),isRead:true})}>
            ✓ Mark as read
          </button>
          <button style={{...S.btnSm,background:"#FFF8DC",color:"#D97706",border:"1px solid rgba(217,119,6,.2)"}}
            onClick={()=>readMutation.mutate({orderIds:Array.from(selected),isRead:false})}>
            ○ Mark as unread
          </button>
          <button style={{...S.btnSm,marginLeft:"auto"}} onClick={()=>setSelected(new Set())}>Clear</button>
        </div>
      )}
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tr>
              <th style={{...S.th,width:40}}>
                <input type="checkbox" checked={allSelected} onChange={toggleAll}
                  style={{width:15,height:15,cursor:"pointer",accentColor:"#88dde1"}} />
              </th>
          ...
          <tbody>
            {isLoading ? (
              ...
              const isUnread = !o.isRead && o.status === "PLACED"
              return (
                <>
                <tr key={o.id} style={{background:isUnread?"#f0fafb":isExpanded?"rgba(136,221,225,.04)":"white",fontWeight:isUnread?600:400}}>
                  <td style={{...S.td,padding:"12px 14px"}}>
                    <input type="checkbox" checked={isSelected} onChange={()=>toggleSelect(o.id)}
                      style={{width:15,height:15,cursor:"pointer",accentColor:"#88dde1"}} />
                  </td>
                  <td style={{...S.td,width:32,padding:"12px 6px"}}>
                    <button onClick={()=>handleExpand(o.id, o.isRead)}
                  ...
                  <td style={S.td}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      {isUnread && <span style={{width:7,height:7,borderRadius:"50%",background:"#e11d48",flexShrink:0,display:"inline-block"}} />}
                      <span style={{fontWeight:600,color:"#1a9da3"}}>{o.orderNumber}</span>
                    </div>
                  </td>
                  <td style={S.td}>
                ...
                {isExpanded && (
                  <tr key={o.id+"-detail"}>
                    <td colSpan={9} style={{padding:0,borderBottom:"1px solid rgba(0,0,0,.06)"}}>
                      <div style={{background:"#FAFBFC",borderTop:"1px solid rgba(136,221,225,.2)",padding:"16px 24px"}}>
                        {!orderDetail ? (
                          <p style={{color:"#8888AA",fontSize:13,margin:0}}>Loading…</p>
                        ) : (
                          <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:24}}>
                            <div>
                              <p style={{fontSize:11,fontWeight:700,textTransform:"uppercase" as const,letterSpacing:".07em",color:"#8888AA",margin:"0 0 10px"}}>Order Lines</p>
                              <div style={{background:"white",border:"1px solid rgba(0,0,0,.08)",borderRadius:10,overflow:"hidden"}}>
                                    ...
                                    {editingItem?.id === item.id ? (
                                      <div>
                                        ...
                                        <div style={{display:"flex",gap:12,alignItems:"flex-end",flexWrap:"wrap" as const}}>
                                          <div>
                                            <div style={{fontSize:10,color:"#8888AA",marginBottom:4,textTransform:"uppercase" as const,fontWeight:600}}>Unit Cost (£)</div>
                                            <input style={{...S.inputSm,width:90}} type="number" step="0.01" value={editingItem.unitCost} onChange={e=>setEditingItem(ei=>ei?{...ei,unitCost:e.target.value}:ei)} />
                                          </div>
                                          <div>
                                            <div style={{fontSize:10,color:"#8888AA",marginBottom:4,textTransform:"uppercase" as const,fontWeight:600}}>Quantity</div>
                                            <input style={{...S.inputSm,width:70}} type="number" min="1" value={editingItem.quantity} onChange={e=>setEditingItem(ei=>ei?{...ei,quantity:e.target.value}:ei)} />
                                          </div>
                                        ...
                                        <div style={{display:"flex",gap:8,marginTop:10}}>
                                          <button style={S.btnPink} onClick={()=>editItemMutation.mutate({orderId:o.id,itemId:item.id,unitCostPence:Math.round(parseFloat(editingItem.unitCost)*100),quantity:parseInt(editingItem.quantity)})} disabled={editItemMutation.isPending}>
                                      ...
                                      <div style={{display:"flex",alignItems:"center",gap:12}}>
                                        ...
                                        <div style={{display:"flex",gap:6,flexShrink:0}}>
                                          <button style={S.btnSm} onClick={()=>setEditingItem({id:item.id,unitCost:(item.unitCostPence/100).toFixed(2),quantity:String(item.quantity)})}>✏️</button>
                                ...
                                <div style={{padding:"10px 14px",borderTop:"1px solid rgba(0,0,0,.08)",background:"#FAFBFC"}}>
                                  <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8888AA",marginBottom:3}}><span>Subtotal</span><span>{formatCurrency(orderDetail.subtotalPence)}</span></div>
                                  <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8888AA",marginBottom:5}}><span>VAT (20%)</span><span>{formatCurrency(orderDetail.vatPence)}</span></div>
                                  <div style={{display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:700,color:"#1a9da3"}}><span>Total</span><span>{formatCurrency(orderDetail.totalPence)}</span></div>
                                </div>
                              </div>
                            </div>
                            <div>
                              ...
                              <div style={{background:"white",border:"1px solid rgba(0,0,0,.08)",borderRadius:10,padding:"12px 14px"}}>
                                {[
                                  ["Retailer",orderDetail.retailer?.businessName],
                                  ["Placed",formatDateTime(orderDetail.createdAt)],
                                  ["PO Ref",orderDetail.poReference??"—"],
      ...
      {editing && (
        <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setEditing(null)}>
          <div style={S.modal}>
            ...
            <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,fontWeight:600,color:"#4A4A6A",marginBottom:5,textTransform:"uppercase" as const}}>Note (optional)</label><input style={S.input} value={note} onChange={e=>setNote(e.target.value)} /></div>
